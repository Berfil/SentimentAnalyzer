...
with st.sidebar:
    ...
    source = st.selectbox(
        "データソース (Source)",
        options=["yahoo", "cosme", "tabelog", "kakaku", "all"],
    ...
    with col2:
        max_items = st.number_input(
            "製品/店舗数" if source != "yahoo" else "（無効）",
            min_value=1, max_value=20, value=5,
            disabled=(source == "yahoo"),
...
tab_pos, tab_neg, tab_neu = st.tabs(["😊 ポジティブ Top 5", "😞 ネガティブ Top 5", "😐 ニュートラル Top 5"])
...
show_top("positive", tab_pos)
show_top("negative", tab_neg)
show_top("neutral", tab_neu)
